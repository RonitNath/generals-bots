"""
CLI entry point for hosting a LAN game server.

Installed as ``generals-server`` via pyproject.toml console scripts.
"""
...
def main(argv: list[str] | None = None):
    parser = argparse.ArgumentParser(
        description="Host a Generals LAN game server with web spectator UI",
    )
    parser.add_argument("--host", type=str, default="0.0.0.0", help="Interface to bind")
    ...
    parser.add_argument("--ctl-port", type=int, default=5556, help="TCP control port for remote CLI")
    parser.add_argument("--no-spectator", action="store_true", help="Disable web spectator UI")
    parser.add_argument("--min-distance", type=int, default=20, help="Min Manhattan distance between generals")
