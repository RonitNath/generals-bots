...
class AnomalyEngine:
    ...
    def detect(
        ...
        for idx, (obs, action, action_kind, debug) in enumerate(zip(observations, actions, action_kinds, debug_snapshots, strict=True)):
            ...
            if bool(action_kind["pass"]) and max_army >= 20:
                detections.append(Detection(idx, "large_army_pass", 4, 8.0, {"max_owned_army": max_army}))

            if action_kind["kind"] == "attack_city" and int(obs.owned_army_count) < int(obs.opponent_army_count):
                detections.append(
                    Detection(
                        idx,
            ...
            if (int(obs.timestep) + 1) % 50 == 0 and action_kind["kind"] in {"pass", "reinforce"} and max_army >= 8:
                detections.append(Detection(idx, "income_timing_miss", 2, 2.0, {"max_owned_army": max_army}))

            if action_kind["kind"] == "reinforce" and int(obs.opponent_army_count) > int(obs.owned_army_count) * 1.2:
            ...
            if action_kind["kind"] == "pass" and len(history.max_armies) >= 3 and min(history.max_armies) >= 15:
                detections.append(Detection(idx, "idle_strike_force", 3, 5.0, {"window": len(history.max_armies)}))

            if action_kind["kind"] == "reinforce" and max_army >= 25 and int(obs.opponent_land_count) >= int(obs.owned_land_count):
                detections.append(Detection(idx, "frontline_retreat_without_threat", 2, 2.5, {"max_owned_army": max_army}))

            if action_kind["kind"] == "scout" and max_army >= 18 and debug and debug.get("own_general") is not None:
            ...
            if debug and debug.get("enemy_general_estimate") is not None and int(obs.opponent_army_count) > int(obs.owned_army_count) * 1.4:
                chosen = debug.get("chosen")
                own_general = debug.get("own_general")
                if chosen and own_general:
                    if abs(chosen["dest"][0] - own_general[0]) + abs(chosen["dest"][1] - own_general[1]) > 5:
                        detections.append(Detection(idx, "missed_general_threat", 3, 4.0, {"chosen_dest": chosen["dest"]}))

            if debug and debug.get("enemy_general_estimate") is not None and max_army >= 18:
                chosen = debug.get("chosen")
                if chosen:
                    dest = chosen["dest"]
                    ...
                    if candidate_scores and chosen["score"] < max(candidate_scores) - 25:
            ...
            if len(history.moves) >= 4:
                ...
                if recent_kinds.count("attack") + recent_kinds.count("attack_city") == 0 and max_army >= 18:
        ...
        for idx in range(2):
            ...
            if int(next_state["army"][1 - idx]) - int(prev_state["army"][1 - idx]) >= 8 and action_kinds[idx]["kind"] == "reinforce":
